            ...
            <div class="row g-3">
                <div class="col-lg-8">
                    <div class="row g-3">
                        ...
                        {{-- RIDER NOT FOUND --}}
                        @if (is_null($trip->driver) && $trip->current_status == PENDING && $trip->ride_request_type != SCHEDULED)
                            <div class="col-sm-6">
                                <div class="card border analytical_data h-100">
                                    <div class="card-body position-relative">
                        ...
                        @endif
                        @if ($trip->ride_request_type == SCHEDULED && $trip->current_status == PENDING)
                            <div class="col-sm-6">
                                <div class="card border analytical_data h-100">
                                    <div class="card-body position-relative">
                                        <div class="d-flex flex-column  gap-4">
                                            ...
                                            <p class="bg-F1F7FF text-capitalize text-center p-3 bg-grey w-50 mx-auto mb-0 rounded">{{ translate('No_Rider_Assigned_Yet') }}
                                            </p>
                                        </div>
                                    </div>
                                </div>
                        ...
                        {{-- REJECTED BY DRIVERS --}}
                        @if($trip->rejectedDriverRequests->isNotEmpty())
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        <h6 class="d-flex align-items-center gap-2 text-capitalize mb-3">
                                            <i class="bi bi-x-circle-fill text-danger"></i>
                                            {{ translate('Rejected by Drivers') }}
                                            <span class="badge bg-danger">{{ $trip->rejectedDriverRequests->count() }}</span>
                                        </h6>
                                        <div class="table-responsive">
                                            <table class="table table-sm table-borderless align-middle mb-0">
                                                <thead class="bg-light">
                                                    <tr>
                                                        <th class="text-capitalize fs-12">#</th>
                                                        <th class="text-capitalize fs-12">{{ translate('Driver') }}</th>
                                                        <th class="text-capitalize fs-12">{{ translate('Phone') }}</th>
                                                        <th class="text-capitalize fs-12">{{ translate('Rejected At') }}</th>
                                                    </tr>
                                                </thead>
                                                <tbody>
                                                    @foreach($trip->rejectedDriverRequests as $index => $rejection)
                                                        <tr>
                                                            <td class="fs-12">{{ $index + 1 }}</td>
                                                            <td>
                                                                <div class="d-flex align-items-center gap-2">
                                                                    <div class="avatar avatar-sm rounded-circle">
                                                                        <img src="{{ onErrorImage(
                                                                            $rejection->user?->profile_image,
                                                                            dynamicStorage('storage/app/public/driver/profile') . '/' . $rejection->user?->profile_image,
                                                                            dynamicAsset('public/assets/admin-module/img/avatar/avatar.png'),
                                                                            'driver/profile/',
                                                                        ) }}" class="rounded-circle fit-object" width="32" height="32" alt="">
                                                                    </div>
                                                                    <span class="fs-12">{{ $rejection->user?->first_name }} {{ $rejection->user?->last_name }}</span>
                                                                </div>
                                                            </td>
                                                            <td class="fs-12">{{ $rejection->user?->phone ?? '—' }}</td>
                                                            <td class="fs-12" dir="ltr">{{ date('d M Y, h:i a', strtotime($rejection->created_at)) }}</td>
                                                        </tr>
                                                    @endforeach
                                                </tbody>
                                            </table>
                        ...
                        @endif
                        {{-- END REJECTED BY DRIVERS --}}

                        {{-- TRIP SUMMARY FOR PENDING OR CANCELLED RIDE --}}
                        @if (in_array($trip->current_status, [PENDING, ACCEPTED, ONGOING, OUT_FOR_PICKUP]))
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                            ...
                                            <div class="d-flex flex-column gap-1">
                                                <div
                                                    ...
                                                    <h6
                                                        ...
                                                        {{ translate($trip->current_status == PENDING && $trip->ride_request_type == SCHEDULED ? SCHEDULED : $trip->current_status)  }}</h6>
                                                </div>
                                                <div
                                                    class="d-flex align-items-center justify-content-sm-end gap-3 {{ $trip->ride_request_type == SCHEDULED ? 'gap-sm-2' : 'gap-sm-0' }} ">
                                                    <div class=" text-capitalize">{{ translate('trip_type') }}:</div>
                                                    <h6 class="fs-12 {{ $trip->ride_request_type == SCHEDULED ? '' : 'w-100p' }} text-sm-end text-capitalize">
                                                        {{ translate($trip->type) }}
                        ...
                        @if ($trip->current_status == COMPLETED || $trip->current_status == CANCELLED || $trip->current_status == RETURNED || $trip->current_status == RETURNING)
                            <div class="col-12">
                                <div class="card">
                                    <div class="card-body">
                                        @if(!$trip->parcelRefund)
                                        ...
                                        <div class="px-xl-3">
                                          <dl class="data-list gy-2 text-dark">
                                              ...
                                              <dt class="text-capitalize">{{ translate('booking_fee') }}</dt>
                                              ...
                                              <dt class="ps-custom-4">
                                                  <div class="d-flex align-items-center gap-2 text-capitalize">
                                                      ...
                                                      <i class="bi bi-info-circle-fill text-primary tooltip-icon" data-bs-toggle="tooltip" data-bs-title="{{ translate('the_Fee(in_percentage)_charged_from_the_customer_to_cancel_the_trip') }}">
                                                      </i>
                                                  </div>
                                              </dt>
                                              <dd class="m-0">+ {{ set_currency_symbol($trip?->fee?->cancellation_fee) }}</dd>
                                              @endif

                                              @if($trip?->extra_fare_fee > 0)
                                              <dt class="text-capitalize">
                                                  {{ translate('extra_fare') }}
                                                  <i class="bi bi-info-circle-fill text-primary tooltip-icon" data-bs-toggle="tooltip" data-bs-title="{{ translate('This charge is added to a ride for special conditions, such as extreme weather.') }}">
                                                  </i>
                                              </dt>
                                              <dd class="m-0">+{{ set_currency_symbol($trip->extra_fare_amount + 0) }}</dd>
                                              @endif

                                              @if ($trip->tips > 0)
                                              <dt>
                                                  {{ translate('tips') }}
                                                  <i class="bi bi-info-circle-fill text-primary tooltip-icon" data-bs-toggle="tooltip" data-bs-title="{{ translate('This amount have got the driver from the customer as reward for his trip complete.') }}">
                                                  </i>
                                              </dt>
                                              <dd class="m-0">+ {{ set_currency_symbol($trip->tips + 0) }}</dd>
                                              @endif

                                              <dt><strong>{{ translate('total') }}</strong> ({{ translate('paid') }})</dt>
                                              <dd class="m-0">
                                                  <strong>{{ set_currency_symbol($trip->paid_fare - ($trip->type == 'parcel' && ($trip->current_status == RETURNED || $trip->current_status == RETURNING) ? $trip?->return_fee : 0)) }}</strong>
                                              </dd>

                                              @if ($trip->type == 'parcel' && ($trip->current_status == RETURNED || $trip->current_status == RETURNING) && $trip->return_fee > 0)
                                              <dt class="ps-custom-4">
                                                  <div class="d-flex align-items-center gap-2 text-capitalize">
                                                      {{ translate('return_fee') }} ({{ $trip->current_status == RETURNING ? translate('due') : translate('paid') }})
                                                      <i class="bi bi-info-circle-fill text-primary tooltip-icon" data-bs-toggle="tooltip" data-bs-title="{{ translate('the_Fee(in_percentage)_charged_from_the_customer_to_cancel_the_trip') }}">
                                                      </i>
                                                  </div>
                                              </dt>
                                              <dd class="m-0">+ {{ set_currency_symbol($trip?->return_fee) }}</dd>
                                              @endif
                                          </dl>
