...
<div class="col-lg-4">
                        ...
                        @if($safetyAlert?->status == PENDING)
                            <div class="d-flex gap-2 justify-content-between flex-wrap">
                                ...
                                <form action="{{ route('admin.safety-alert.mark-as-solved', $safetyAlert->id) }}"
                                    ...
                                    @csrf
                                    @method('PUT')
                                    <button type="submit" class="btn btn-primary m-0 p-0">
    ...
    <div class="card mb-3">
        <div class="card-body">
            <h5 class="text-center mb-3 text-capitalize">{{ translate('Update Trip') }}</h5>
            ...
            <form action="{{ route('admin.trip.update', $trip->id) }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="current_status" class="js-select cmn_focus form-select">
                        @foreach([PENDING, ACCEPTED, ONGOING, COMPLETED, CANCELLED] as $status)
                            <option value="{{ $status }}"
                                {{ $trip->current_status == $status ? 'selected' : '' }}>
                                {{ translate($status) }}
                            </option>
                        @endforeach
                    </select>
                ...
                {{-- Payment Status --}}
                <div class="mb-3">
                    <label class="form-label">{{ translate('payment_status') }}</label>
                    <select name="payment_status" class="js-select cmn_focus form-select">
                        @foreach([PAID, UNPAID, 'partial'] as $pStatus)
                            <option value="{{ $pStatus }}"
                                {{ $trip->payment_status == $pStatus ? 'selected' : '' }}>
                                {{ translate($pStatus) }}
                            </option>
                        @endforeach
                    </select>
                ...
                <div class="mb-4">
                    ...
                    <select name="driver_id" class="js-select cmn_focus form-select">
                        <option value="">— {{ translate('Unassigned') }} —</option>
                        @php
                            $availableDrivers = \Modules\UserManagement\Entities\User::where('user_type', DRIVER)
                                ->where('is_active', 1)
                                ->get();
                        @endphp
                ...
                <button type="submit" class="btn btn-primary w-100 cmn_focus">
                ...
                <ul class="list-icon">
                    <li>
                        <div class="media gap-2">
                            <img width="18" src="{{dynamicAsset('public/assets/admin-module/img/svg/gps.svg')}}"
                                 class="svg" alt="">
                        ...
                        <div class="media gap-2">
                            ...
                            @if($trip->current_status == 'completed')
                                <div class="media-body text-capitalize">
                                    {{translate('total_distance')}} - {{$trip->actual_distance}} {{translate('km')}}
                                </div>
                            @else
                                <div class="media-body text-capitalize">
                                    {{translate('total_distance')}} - {{$trip->estimated_distance}} {{translate('km')}}
                                </div>
                            @endif
                        </div>
                    </li>
                        ...
                        <div class="border rounded border-ced4da p-3 mb-4">
                            <div class="d-flex flex-wrap gap-5">
                                ...
                                <div>
                                    ...
                                    <label class="form-check-label" for="create-refund-coupon">Create a refund Coupon</label>
                                </div>
                            </div>
                        </div>
                        <div class="mb-4">
