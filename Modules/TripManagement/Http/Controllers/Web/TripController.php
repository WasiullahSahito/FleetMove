...
class TripController extends BaseController
{
    ...
    public function show($id, Request $request): \Illuminate\Contracts\Foundation\Application|\Illuminate\Contracts\View\Factory|\Illuminate\Contracts\View\View|RedirectResponse
    {
        ...
        $trip = $this->tripRequestService->findOne(id: $id, relations: ['customer', 'driver', 'tripStatus', 'parcelRefund.refundProofs', 'parcel.parcelCategory', 'rejectedDriverRequests.user'], withTrashed: true);
    ...
    public function export(Request $request): View|Factory|Response|StreamedResponse|string|Application
    {
        ...
        $data = $trips->map(fn($item) => [
            ...
            'Booking Fee' => $item['current_status'] == 'completed' ? getCurrencyFormat($item['actual_fare'] ?? 0) : getCurrencyFormat($item['estimated_fare'] ?? 0),
    ...
    public function update(Request $request, $id): RedirectResponse
{
    ...
    if ($request->has('driver_id')) {
        $data['driver_id'] = $request->driver_id ?: null;
    }

    if (!empty($data)) {
        $this->tripRequestService->update(id: $id, data: $data);
