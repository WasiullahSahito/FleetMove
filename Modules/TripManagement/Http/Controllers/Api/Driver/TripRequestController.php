    ...
    public function rideStatusUpdate(Request $request)
    {
        ...
        if (!$data) {
            return response()->json(responseFormatter(constant: [
                'response_code' => 'drop_off_location_not_found_404',
                'message' => translate('Drop off location not found')
            ]), 403);
        }
    ...
    public function requestAction(Request $request): JsonResponse
    {
        ...
        if ($request['action'] != ACCEPTED) {
            ...
            $this->adminNotificationService->create([
                'model' => 'trip_request',
                'model_id' => $trip->id,
                'message' => 'driver_rejected_ride_request',
            ]);
            return response()->json(responseFormatter(constant: DEFAULT_UPDATE_200));
        }
        $env = env('APP_MODE');
        ...
        if (array_key_exists('error', $driverArrivalTime)) {
            return response()->json(responseFormatter(constant: [
                ...
                'message' => translate('No available route from your current location to the pickup address.')
            ]), 403);
        }
