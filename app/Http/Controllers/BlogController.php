...
class BlogController extends Controller
{
    ...
    {
        // FIX: commonData() returns a RedirectResponse when blog is disabled.
        // We must check and return it directly instead of passing to view().
    ...
    {
        // FIX: same guard — return the redirect if blog is disabled.
    ...
    /**
     * Returns an array of view data, OR a RedirectResponse if blog is disabled.
     * Callers MUST check: if ($data instanceof RedirectResponse) return $data;
     */
    protected function commonData(Request $request, ?string $slug = null): array|\Illuminate\Http\RedirectResponse
    {
        $isEnabled = $this->blogSettingService->findOneBy(criteria: ['key_name' => 'is_enabled', 'settings_type' => BLOG_PAGE])?->value;

        if (empty($isEnabled)) {
            Toastr::error(message: BLOG_DISABLE['message'], title: translate(BLOG_DISABLE['response_code']));
            // FIX: return the redirect so callers can propagate it properly.
            return redirect('/');
        }
