...
// --- FIX: Added missing getCurrencySymbol() used by Book Now blade view ---
if (!function_exists('getCurrencySymbol')) {
    function getCurrencySymbol(): string
    {
        ...
        return businessConfig('currency_symbol', 'business_information')->value ?? "$";
    }
}
...
if (!function_exists('fileUploader')) {
    function fileUploader(string $dir, string $format= APPLICATION_IMAGE_FORMAT, ?UploadedFile $image = null, $oldImage = null): string|false
    {
        ...
        // --- FIX: Prevent 128MB Memory Exhaustion on large images ---
        ...
        if ($width > $maxSize || $height > $maxSize) {
            ...
            imagecopyresampled($temp, $gdImage, 0, 0, 0, 0, $newW, $newH, $width, $height);
        ...
        if ($saved)
        {
            foreach ((array) $oldImage as $file) {
                if (!empty($file) && Storage::disk('public')->exists($dir . $file)) {
...
if (!function_exists('referralEarningSetting')) {
    ...
    {
        try {
            return ReferralEarningSetting::query()
                ->where('key_name', $key)
                ->when($settingsType, function ($query) use ($settingsType) {
                    $query->where('settings_type', $settingsType);
                })
...
if (!function_exists('externalConfig')) {
    ...
    {
        try {
            return ExternalConfiguration::query()
                ->where('key', $key)
                ->first();
        } catch (Exception) {
            return null;
...
if (!function_exists('configSettings')) {
    ...
    {
        try {
            return DB::table('settings')->where('key_name', $key)
...
if (!function_exists('getExtensionIcon')) {
    // --- FIX: Parameter changed from `string` to `?string` so that drivers
    // who have not uploaded a document yet (null value) do not crash the
    // admin driver/edit.blade.php view with:
    // "getExtensionIcon(): Argument #1 ($document) must be of type string, null given"
    function getExtensionIcon(?string $document): string
    {
        // Return the generic upload placeholder whenever there is no document.
        if (empty($document)) {
...
// --- FIX: Prevent crash if $type is null ---
if (!function_exists('convertTimeToSecond')) {
    function convertTimeToSecond(int|float|string|null $time, ?string $type = null): ?float
    {
...
if (!function_exists('sendDeviceNotification')) {
    function sendDeviceNotification($fcm_token, $title, $description, $status = null, $image = null, $ride_request_id = null, $type = null, $notification_type = null, $action = null, $user_id = null)
    {
        try {
            if ($user_id) {
            ...
            if (!$fcm_token || strlen($fcm_token) < 20 || $fcm_token === '@') {
                return false;
            }
            // --- FIX: Safely parse JSON from cache or DB to prevent stdClass error ---
            $cachedKey = \Illuminate\Support\Facades\Cache::get('server_key');
            if ($cachedKey) {
                $serviceAccount = is_string($cachedKey) ? json_decode($cachedKey, true) : json_decode(json_encode($cachedKey), true);
            } else {
                $setting = \Illuminate\Support\Facades\DB::table('business_settings')->where('key_name', 'server_key')->first();
                if (!$setting || empty($setting->value)) return false;

                $level1 = is_string($setting->value) ? json_decode($setting->value, true) : json_decode(json_encode($setting->value), true);
                $serviceAccount = is_string($level1) ? json_decode($level1, true) : json_decode(json_encode($level1), true);
            }

            if (empty($serviceAccount['project_id']) || empty($serviceAccount['private_key'])) {
                \Illuminate\Support\Facades\Log::error("FCM Send: Missing project_id or private_key in server_key config.");
                return false;
            }

            // --- FIX: Prevent "Array to string conversion" by validating cache extraction ---
            $accessToken = \Illuminate\Support\Facades\Cache::get('firebase_access_token');

            // Safety measure: if an old/bugged array was saved to cache, wipe it.
            ...
            if (!$accessToken || !is_string($accessToken)) {
                ...
                if (!$key) {
                    \Illuminate\Support\Facades\Log::error("FCM Send: Invalid private key format.");
                    return false;
                }
                ...
                $resp = \Illuminate\Support\Facades\Http::asForm()->post('https://oauth2.googleapis.com/token', [
                    'grant_type' => 'urn:ietf:params:oauth:grant-type:jwt-bearer',
                    'assertion'  => $jwt,
                ]);

                if (!$resp->successful() || empty($resp->json('access_token'))) {
                    \Illuminate\Support\Facades\Log::error("FCM Send: Failed to generate token: " . $resp->body());
                    return false;
                }

                // Get strictly the string token, never the array
                $accessToken = (string) $resp->json('access_token');

                \Illuminate\Support\Facades\Cache::put('firebase_access_token', $accessToken, now()->addMinutes(55));
            }

            $fcmUrl = "https://fcm.googleapis.com/v1/projects/{$serviceAccount['project_id']}/messages:send";

            // --- FIX: Cast safely to avoid further Array to string warnings ---
            $payload = [
                'message' => [
                    'token' => $fcm_token,
                    'notification' => [
                        'title' => is_scalar($title) ? (string)$title : '',
                        'body'  => is_scalar($description) ? (string)$description : '',
                    ],
                    'data' => [
                        'title'             => is_scalar($title) ? (string)$title : '',
                        'body'              => is_scalar($description) ? (string)$description : '',
                        'trip_request_id'   => is_scalar($ride_request_id) ? (string)$ride_request_id : '',
                        'ride_request_id'   => is_scalar($ride_request_id) ? (string)$ride_request_id : '',
                        'type'              => is_scalar($type) ? (string)$type : '',
                        'action'            => is_scalar($action) ? (string)$action : '',
                        'notification_type' => is_scalar($notification_type) ? (string)$notification_type : '',
            ...
            if ($image && is_scalar($image)) {
                $payload['message']['notification']['image'] = (string)$image;
            }

            // At this point $accessToken is 100% a string — solves PendingRequest line 528 error.

            $response = \Illuminate\Support\Facades\Http::withToken($accessToken)->timeout(10)->post($fcmUrl, $payload);
            ...
            if (!$response->successful()) {
                ...
                if ($isUnregistered || $response->status() === 404) {
                    ...
                    \Illuminate\Support\Facades\Log::info("FCM Cleanup: Token removed for user_id: {$user_id} due to UNREGISTERED error.");
                } else {
                    \Illuminate\Support\Facades\Log::error("FCM Send Failed: " . $response->body());
                }
            ...
            \Illuminate\Support\Facades\Log::error("FCM Helper Error: " . $e->getMessage() . " in " . $e->getFile() . " on line " . $e->getLine());
            return false;
        }
    }
}
